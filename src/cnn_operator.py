...
def activation_layer_addition(model, add_position):
    ...
    :return: mutant model
    """
    activation_list = ["tanh", "softmax", "elu", "selu", "softplus", "softsign", "relu", "sigmoid", "hard_sigmoid", "linear"]
...
def duplicate_layer_addition(model, layer_name):
    ...
    print(type(model_config["layers"]))
    i = 0
    # exit()
    layer = None
    ind = -1
    for x in model_config["layers"]:
    ...
    if layer is not None:
        model_config["layers"].insert(ind, layer)
