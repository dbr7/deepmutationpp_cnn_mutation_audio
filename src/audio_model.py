...
def get_test_df(type):
    if type == 'T':
        ...
        DATASET_ROOT = os.path.join('..', '..', dataset_path, "Audio", "16000_pcm_speeches")
        ...
        # Seed to use when shuffling the dataset and the noise
        SHUFFLE_SEED = 43
        easy_x = np.load(os.path.join('..', '..', dataset_path, "Audio", "audio_easy_x.npy"))
        easy_y = np.load(os.path.join('..', '..', dataset_path, "Audio", "audio_easy_y.npy"))

        test_ds_new = tf.data.Dataset.zip(
            (tf.data.Dataset.from_tensor_slices(easy_x), tf.data.Dataset.from_tensor_slices(easy_y)))
        test_ds_new = test_ds_new.shuffle(buffer_size=32 * 8, seed=SHUFFLE_SEED).batch(32)
        test_ds_new = test_ds_new.prefetch(tf.data.experimental.AUTOTUNE)
